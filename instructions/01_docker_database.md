...
## Objective
Set up a Docker Compose PostgreSQL database, define the complete schema, and create a thin async repository layer that all other backend modules will use.
...
services:
  db:
    ...
    ports:
      - "5432:5432"
    environment:
      POSTGRES_USER: fitnova
      POSTGRES_PASSWORD: fitnova_dev
      POSTGRES_DB: fitnova
    volumes:
...
Key points:
- The `schema.sql` is auto-executed on first container creation via `docker-entrypoint-initdb.d`.
- Port 5432 exposed to host.

### 2. `backend/db/__init__.py`
Empty file.

### 3. `backend/db/schema.sql`
Copy the FULL schema from `instructions/project_description.md` — the section titled "Database Schema (PostgreSQL)". This includes:
- `organizations`, `teams`, `advisors` tables (org hierarchy)
- `calls`, `transcripts`, `reports` tables (call processing)
- `flag_reviews` table (feedback loop)
- All seed data (FitNova org, Pod Alpha, Pod Beta, 6 advisors)
- All indexes

### 4. `backend/db/connection.py`
...
All database read/write functions. Use `asyncpg` with parameterized queries. Every function takes a `pool: asyncpg.Pool` as its first argument.
...
2. Schema is auto-applied with seed data
3. `SELECT * FROM advisors;` returns 6 rows
4. All repository functions are type-hinted and use parameterized queries (no SQL injection)
5. Connection pool initializes and closes cleanly
