...
## Objective
Define an ingestion adapter interface so that FitNova can swap telephony/CRM vendors without code changes. Implement the file upload adapter (current approach) and stub out webhook/CRM adapters.

## Parallelization
**Group A** — No dependencies. Can be executed immediately.

## Context
The assignment explicitly requires: *"Design the ingestion layer to be source-agnostic: FitNova may switch telephony or CRM vendors, or run several at once"* and *"a new telephony, dialer, CRM, or file source can be mapped in without code changes."*
...
In `routers/calls.py` (Task 04), the upload endpoint uses this adapter:

```python
from services.ingestion import FileUploadAdapter

# During app startup (lifespan):
app.state.ingest_adapter = FileUploadAdapter(settings.upload_dir)

# In the upload endpoint:
metadata = await request.app.state.ingest_adapter.ingest(
...
1. **ABC pattern**: New sources implement `IngestAdapter.ingest()` and return a standardized `CallMetadata`. The pipeline doesn't care where audio came from.
2. **Stubs with docstrings**: The assignment asks you to *design* for extensibility. The `WebhookAdapter` and `CRMExportAdapter` stubs show the design even though they're not implemented — with clear docstrings explaining what they'd do in production.
3. **Factory registry**: `get_adapter()` maps source names to adapters. Adding a new source = one class + one dict entry.
4. **No external dependencies**: This module is pure Python with no new imports.

## Acceptance Criteria
1. `FileUploadAdapter` correctly saves audio and returns `CallMetadata`
2. Extension validation rejects unsupported formats
3. `CallMetadata` includes all fields the pipeline needs
4. Stubs raise `NotImplementedError` with descriptive messages
5. `get_adapter("file_upload", upload_dir)` returns a working adapter
